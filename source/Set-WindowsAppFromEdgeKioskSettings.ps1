...
Then run the script to configure the kiosk:
...
Installs Windows App, creates KioskUser0 with automatic logon via Assigned Access, configures auto-logoff on idle/close, and uses the default local HTML file.
...
Configures kiosk with custom web portal URL and allows navigation to tailspintoys.com domain. Assigned Access autologon with KioskUser0 is automatically configured.
...
Full configuration with power management and automatic maintenance. Assigned Access autologon with KioskUser0 is automatically configured.
